...
with open ("/opt/airflow/dags/airflow_inputs.json", "r", encoding = 'utf-8' ) as f:
    ...
    random_names_lst = usr_inputs["random_name"]
...
def Prediction_Inputs(u_name, pr_name, mdl_link, mlangles_s3,source_data, instance_type, random_name):
    user_inputs = {
                    "Type_Of_Instance": instance_type,
                    "Username": u_name ,
                    "Project_Name": pr_name,
                    "model_link": mdl_link,
                    "mlangles_s3_bucketname": mlangles_s3,
...
for k in range(len(dag_ids_lst)):
    ...
    if (type_of_source[0] == 'DataURL'):
        ...
        pred_inputs = Prediction_Inputs(usernames_lst[k], pr_names_lst[k], model_links_lst[k], mlangles_s3_buckets_name_lst, type_of_sources_lst[k], instance_type_lst[k], random_names_lst[k])
        '''dag = DAG(
