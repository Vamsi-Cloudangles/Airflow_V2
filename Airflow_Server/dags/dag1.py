...
def prediction_inputs(u_name, pr_name, mdl_link, mlangles_s3,source_data, instance_type):
    user_inputs = {
...
for k in range(len(dag_ids_lst)):
    ...
    if (type_of_source[0] == 'DataURL'):
        ...
        pred_inputs = prediction_inputs(usernames_lst[k], pr_names_lst[k], model_links_lst[k], mlangles_s3_buckets_name_lst, type_of_sources_lst[k], instance_type_lst[k])
        dag = DAG(
